...
//p.setName(name:"xiaoHan", age:100)    ❌
p.setName(name: "hjq", 88)  //right
p.show()

/*
 self关键字, Swift中的self和OC中的self基本一样;
 self指当前对象, self在对象方法中代表当前对象, 但是在类方法中没有self
 */
class Person2 {
    var name: String = "hjq"
    var age: Int = 25
    ...
    {
        //默认情况下, _name和_age前面默认有一个self关键字, 因为所有变量都需要先定义再使用,
        // 而setName方法中并没有定义过_name和_age, 而是在属性中定义的,
        //所以setName中访问的其实是属性, 编译器默认帮我们在前面加了一个self
        //        _name = name
        //        _age = age
        self.name = name
        self.age = age
...
p2.show()
Person2.classMethod()
Person2.classMethod1()
/*
 mutating方法
 1.修改struct，enum的属性变量：周知swift中结构体(structure)和枚举(enumeration)中是可以包含类方法和实例方法，
    可是官方不建议在实例方法中修改其属性变量(immutable)，
    在func前加入mutating关键字后，使其属性变量可修改(mutable)
*/
struct Person3 {
    var name: String = "hjq"
    var age: Int = 24
